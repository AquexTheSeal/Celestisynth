...
public class CrescentiaItem extends SwordItem implements CSWeapon {
    ...
    public void tickRanged(ItemStack itemStack, Level level, Player player, int animationTimer) {
        ...
        if (animationTimer == 20) {
            if (!level.isClientSide()) {
                SkillCastCrescentiaRanged projectile = CSEntityRegistry.CRESCENTIA_RANGED.get().create(level);
            ...
            for (int i = 0; i < 10; i++) {
                ...
                createCrescentiaFirework(itemStack, level, player, player.getX() + offX, player.getY() + offY, player.getZ() + offZ, true);
    ...
    public void tickMelee(ItemStack itemStack, Level level, Player player, int animationTimer) {
        ...
        if (animationTimer == 15) {
            player.playSound(CSSoundRegistry.CS_WHIRLWIND.get(), 0.35F, 0.5F + new Random().nextFloat());
        }
        ...
        if (animationTimer >= 15 && animationTimer <= 60) {
            ...
            for (Entity entityBatch : entities) {
                if (entityBatch instanceof LivingEntity target) {
                    if (target != player && target.isAlive() && !player.isAlliedTo(target) && target.distanceToSqr(player) < rangeSq) {
                        hurtNoKB(player, target, (float) (CSConfig.COMMON.crescentiaSkillDmg.get() + getSharpnessValue(itemStack, 0.25F)));
                        target.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 20, 2));
                    }
                }
                if (entityBatch instanceof Projectile projectile) {
                    createCrescentiaFirework(itemStack, level, player, projectile.getX(), projectile.getY(), projectile.getZ(), true);
            ...
            if (animationTimer % 30 == 0) {
                player.playSound(CSSoundRegistry.CS_WHIRLWIND.get(), 0.15F, 1.5F);
            }

            if (animationTimer % 3 == 0) {
                if (level.random.nextBoolean()) {
                    CSEffect.createInstance(player, null, CSEffectTypes.CRESCENTIA_STRIKE, calculateXLook(player), -0.3, calculateZLook(player));
                } else {
                    CSEffect.createInstance(player, null, CSEffectTypes.CRESCENTIA_STRIKE_INVERTED, calculateXLook(player), -0.3, calculateZLook(player));
                }
                playRandomBladeSound(player, BASE_WEAPON_EFFECTS.length);
            }
            CSEffect.createInstance(player, null, CSEffectTypes.SOLARIS_AIR_LARGE, 0, -1, 0);

            Random random = new Random();
            float offX = (random.nextFloat() * 16) - 8;
            float offY = (random.nextFloat() * 16) - 8;
            float offZ = (random.nextFloat() * 16) - 8;
            createCrescentiaFirework(itemStack, level, player, player.getX() + offX, player.getY() + offY, player.getZ() + offZ, false);
            if (random.nextBoolean()) {
                createCrescentiaFirework(itemStack, level, player, player.getX() + offZ, player.getY() + offX, player.getZ() + offY, false);
            }
        }
