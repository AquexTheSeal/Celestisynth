...
public class PoltergeistCosmicSteelAttack extends WeaponAttackInstance {
    ...
    public int getCooldown() {
        return CSConfig.COMMON.poltergeistSkillCD.get();
    }
    ...
    public void tickAttack() {
        ...
        if (getTimerProgress() == 20) {
            ...
            if (!player.isOnGround() && !player.getAbilities().flying) {
                player.moveTo(player.getX(), calculateNonCollidingPos(player.level, player.blockPosition()).getY() + 1, player.getZ());
            ...
            for (Entity entityBatch : iterateEntities(player.level, createAABB(player.blockPosition().offset(xx, 1, zz), range))) {
                if (entityBatch instanceof LivingEntity target && target != player && target.isAlive() && !player.isAlliedTo(target)) {
                    hurtNoKB(player, target, isGiantImpact ? (float) (double) CSConfig.COMMON.poltergeistSkillDmg.get() : (float) (CSConfig.COMMON.poltergeistSkillDmg.get() + 4) + getSharpnessValue(getStack(), 1.2F));
            ...
            if (isGiantImpact && !player.level.isClientSide()) {
                SkillCastPoltergeistWard projectile = CSEntityRegistry.POLTERGEIST_WARD.get().create(player.level);
