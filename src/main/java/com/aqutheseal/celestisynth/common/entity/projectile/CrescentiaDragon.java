...
public class CrescentiaDragon extends ThrowableProjectile implements GeoEntity, CSWeaponUtil {
    ...
    public void tick() {
        super.tick();
        for (int i = 0; i < 15; i++) {
            double xx = random.nextGaussian() * 0.05F;
            double yy = random.nextGaussian() * 0.05F;
            double zz = random.nextGaussian() * 0.05F;
            Vec3 direction = position();
            ParticleUtil.sendParticle(level(), ParticleTypes.END_ROD, direction.x(), direction.y() + 0.5, direction.z(), xx, yy, zz);
        }
            ...
            for (Entity entityBatch : entities) {
                if (entityBatch instanceof LivingEntity target) {
                    if (target != getOwner() && target.isAlive()) {
                        initiateAbilityAttack(playerOwner, target, (float) (double) CSConfigManager.COMMON.crescentiaShiftSkillDmg.get(), AttackHurtTypes.RAPID_PIERCE);
        ...
        if (tickCount > 100) {
            level().explode(getOwner(), getX(), getY(), getZ(), 3.0F, Level.ExplosionInteraction.MOB);
