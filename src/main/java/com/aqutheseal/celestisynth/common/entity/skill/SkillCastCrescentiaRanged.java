...
public class SkillCastCrescentiaRanged extends EffectControllerEntity {
    ...
    public void tick() {
        ...
        for (Entity entityBatch : entities) {
            if (entityBatch instanceof LivingEntity target) {
                if (target != ownerPlayer && target.isAlive()) {
                    ...
                    target.hurt(DamageSource.playerAttack(ownerPlayer), (float) ((double) CSConfigManager.COMMON.crescentiaShiftSkillDmg.get()));
        ...
        if (tickCount == 100 || !getLevel().getBlockState(newPos).isAir()) {
            level.explode(ownerPlayer, newX, newY, newZ, 3.0F, Explosion.BlockInteraction.DESTROY);
