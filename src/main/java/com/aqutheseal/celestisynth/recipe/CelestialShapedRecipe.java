...
public class CelestialShapedRecipe extends ShapedRecipe implements CelestialCraftingRecipe {
    ...
    public static class Serializer implements RecipeSerializer<CelestialShapedRecipe> {
        private static final ResourceLocation NAME = new ResourceLocation(Celestisynth.MODID, "celestial_crafting_shaped");

        public CelestialShapedRecipe fromJson(ResourceLocation pRecipeId, JsonObject pJson) {
            String s = GsonHelper.getAsString(pJson, "group", "");
            Map<String, Ingredient> map = CelestialShapedRecipe.keyFromJson(GsonHelper.getAsJsonObject(pJson, "key"));
            String[] astring = CelestialShapedRecipe.shrink(CelestialShapedRecipe.patternFromJson(GsonHelper.getAsJsonArray(pJson, "pattern")));
            int i = astring[0].length();
            int j = astring.length;
            NonNullList<Ingredient> nonnulllist = CelestialShapedRecipe.dissolvePattern(astring, map, i, j);
            ItemStack itemstack = CelestialShapedRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(pJson, "result"));
            return new CelestialShapedRecipe(pRecipeId, s, i, j, nonnulllist, itemstack);
        }
